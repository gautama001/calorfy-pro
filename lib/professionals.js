...
export async function getProfessionalClients() {
    ...
    return (data ?? []).map((row) => ({
        ...
        attentionStatus: row.attention_status ?? 'up_to_date',
...
export function subscribeToProfessionalUpdates(professionalId, onUpdate) {
    const channel = supabase
        ...
        .on('postgres_changes', {
            event: '*', schema: 'public', table: 'professional_client_contexts',
            filter: `professional_id=eq.${professionalId}`,
        }, (payload) => onUpdate({ type: 'professional-context', payload }))
